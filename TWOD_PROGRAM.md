...
| D11 | `step_ssprk2_2d!` sanitises stage 1 at `τ` while the stage state is at `τ+Δ` (SSPRK3 uses `τ+Δ`); only the ν-cap `J^τ = U[iDtau]/τ` reads the time, so the cap is loose by ≤ CFLτ = 5 % | **open, cosmetic** — noted 2026-09-04; fixing it means threading a separate sanitize-time through `_stage_2d!` (the `τ` argument is also the RHS evaluation time, which IS correct at `τ`); do it together with the next timestepper change and re-run the ladder |
