...
| the dilute edge | floors, the vacuum ramp (IS2, 2-D) and the relativistic front at the fireball edge shape the tail: quote from $T > T_{\rm fo}$. **Below $T_{\rm fo}$ the tail does not converge and cannot be tuned clean** — on a Woods–Saxon fireball (τ 0.4 → 3, shear+bulk) the second difference of $u^r$ above freeze-out falls 6.6e-4 → 1.8e-4 → 8.5e-5 at $N_r$ = 400/800/1200 while below it *grows* 1.5e-3 → 2.2e-3 → 9.1e-3 (the ∂_τu^r instability above). 🪤 **The timestep optimum is resolution-dependent and non-monotonic**: at $N_r$ = 1200, CFL 0.20 → 0.05 makes the tail 12× cleaner, but at $N_r$ = 400 the same change makes it **4000× worse** ($\max|u^r|$ = 4.08) and the default 0.20 is best; CFL = 0.02 at $N_r$ = 1200 is 700× worse again. ⛔ Filters are worse still (up to 17×) — they damp the dissipative fields, and it is viscosity that suppresses the front: the ideal run is ~200× wigglier there. 🔑 Above $T_{\rm fo}$ the profile is **insensitive to the step** (6.84e-04 at CFL = 0.20/0.15/0.10/0.05), so the default is already converged and a smaller step buys only wall time |
