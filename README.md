...
| the dilute edge | floors, the vacuum ramp (IS2, 2-D) and the relativistic front at the fireball edge shape the tail: quote from $T > T_{\rm fo}$. **Below $T_{\rm fo}$ the tail does not converge** — on a Woods–Saxon fireball (τ 0.4 → 3, shear+bulk) the second difference of $u^r$ above freeze-out falls 6.6e-4 → 1.8e-4 → 8.5e-5 at $N_r$ = 400/800/1200 while below it *grows* 1.5e-3 → 2.2e-3 → 9.1e-3 (the ∂_τu^r instability above), so **refining alone makes the tail worse**. It can be **reduced** only by refining **and** shortening the step together ($N_r$ = 800 with `CFL` = 0.08 gives 6.9e-4, 3.2× better than $N_r$ = 400 at the default) — ⚠ **never removed**: a kink survives at r ≈ 13 where $u^r$ falls 0.460 → 0.386 over one cell and the grid-scale warning still fires. 🪤 **The optimum is resolution-dependent and the curve has spikes, not a basin** — at $N_r$ = 400 the default 0.20 is best and 0.05 is 4000× worse ($\max|u^r|$ = 4.08); at $N_r$ = 1200 the best is 0.07 while 0.15, 0.03 and 0.02 blow up to $\max|u^r|$ ≈ 2.8–4.1 between neighbours that are fine. **Scan before trusting a pair.** ⛔ Filters are worse still (up to 17×) — they damp the dissipative fields, and it is viscosity that suppresses the front: the ideal run is ~200× wigglier there. 🔑 Above $T_{\rm fo}$ none of it matters (1.8e-4 across every `CFL` tried), so production runs gain nothing from a smaller step |
