...
|---|---|---|
| `crosscheck_1p1d.png` | `FiVoFluidumComparison/plot_examples_1p1d.jl` → `figures1d/cmp1d_examples.png` | FiVo vs Fluidum in 1+1D on the worked-example configurations: Bjorken against one closed form, the operator-split ladder, the fireball profiles (drawn to r = 9 fm, the validity edge: |π|/P crosses 1 at 8.29 fm), the three-sector convergence ladder, the charm current. Profiles at N = 800, CFL = 0.08; the ladder stays at the default step |
...
#   CMP1D_RMAX_PLOT=14 shows the tail beyond the validity edge; CMP1D_NPROFILE / CMP1D_CFL
#   override the profile panels' (N, CFL) pair.
